...
export function RestaurantForm({ restaurant, onClose }: RestaurantFormProps) {
  const [name, setName] = useState(restaurant?.name || "")
  const [description, setDescription] = useState(restaurant?.description || "")
  const [icon, setIcon] = useState(restaurant?.icon || "Utensils")
  const [image, setImage] = useState<File | null>(null)
  const [imagePath, setImagePath] = useState(restaurant?.image_path || "")
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (image) {
        try {
          const fileName = `restaurants/${Date.now()}-${image.name.replace(/[^a-zA-Z0-9.]/g, '_')}` // ファイル名を安全に
          ...
          // 古い画像を削除（更新の場合）
          if (restaurant?.image_path && restaurant.image_path !== fileName) {
            const { error: deleteError } = await supabase.storage
              .from("cms-images")
              .remove([restaurant.image_path])
      ...
      if (restaurant) {
        // 更新
        const { error } = await supabase.from("restaurants").update(restaurantData).eq("id", restaurant.id)

        if (error) throw error

        toast({
          title: "更新成功",
          description: "飲食店を更新しました",
        })
      } else {
        // 新規作成
  ...
  return (
    <Dialog open={true} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{restaurant ? "飲食店を編集" : "新規飲食店"}</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="飲食店名" required />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="https://example.com/restaurant-details"
            />
