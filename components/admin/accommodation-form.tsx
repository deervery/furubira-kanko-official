...
export function AccommodationForm({ accommodation, onClose }: AccommodationFormProps) {
  ...
  const [icon, setIcon] = useState(accommodation?.icon || "BedDouble")
  const [image, setImage] = useState<File | null>(null)
  const [imagePath, setImagePath] = useState(accommodation?.image_path || "")
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (image) {
        try {
          const fileName = `accommodations/${Date.now()}-${image.name.replace(/[^a-zA-Z0-9.]/g, '_')}` // ファイル名を安全に
          ...
          // 古い画像を削除（更新の場合）
          if (accommodation?.image_path && accommodation.image_path !== fileName) {
            const { error: deleteError } = await supabase.storage
              .from("cms-images")
              .remove([accommodation.image_path])
      ...
      if (accommodation) {
        // 更新
        const { error } = await supabase.from("accommodations").update(accommodationData).eq("id", accommodation.id)

        if (error) throw error

        toast({
          title: "更新成功",
          description: "宿泊施設を更新しました",
        })
      } else {
        // 新規作成
  ...
  return (
    <Dialog open={true} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{accommodation ? "宿泊施設を編集" : "新規宿泊施設"}</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="https://example.com/accommodation-details"
            />
